 ...
 * @brief: Constructor
 */
RegularP1_L0Mesh_1D::RegularP1_L0Mesh_1D(const Mesh1D &mesh) :
    DiscreteSpaceMesh(mesh),
 ...
 * @brief: Destructor
 */
RegularP1_L0Mesh_1D::~RegularP1_L0Mesh_1D(void)
{
}
 ...
 * @brief: Testing against a base element.
 */
BEM::Complex RegularP1_L0Mesh_1D::testAgainstBaseElement(const ScalarFunctionBase_2D &function, unsigned globalNumber) const
{
    auto &basis = basisFunction(globalNumber);
    BEM::Complex result = 0.0;
    for (auto &i : basis.support()) {
 ...
 * This is the only way to generate a discretization/construct a discrete function.
 */
std::unique_ptr<DiscreteFunctionMesh> RegularP1_L0Mesh_1D::generateFunction(const std::vector<BEM::Complex> coefficients) const
{
    return std::unique_ptr<DiscreteFunctionMesh>(new P1Function(_mesh, coefficients, *_integrator1D));
}
 ...
 * @brief: Given a function, return the function with the same dof
 */
std::unique_ptr<DiscreteFunctionMesh> RegularP1_L0Mesh_1D::generateFunction(const ScalarFunctionBase_2D &function) const
{
    std::vector<BEM::Complex> coef(_size, 0.0);
    for (unsigned i = 1; i <= _mesh.numElements(); i++) {
        auto point = _mesh.point(i);
 ...
 */
/*TODO: CONSIDER RETURNING POINTER TO SCALARFUNCTIONBASE_1D*/
const BasisFunctionMesh &RegularP1_L0Mesh_1D::basisFunction(const unsigned globalNumber) const
{
    // Check numbering. Throw exception if out of bounds.
    if (not (globalNumber < _mesh.numElements() and globalNumber >= 0)) {
        throw std::invalid_argument("Invalid basis index (out of bounds)");
 ...
 * @brief: Construct a discrete function from given coefficients.
 */
RegularP1_L0Mesh_1D::P1Function::P1Function(const Mesh1D &mesh, const std::vector<BEM::Complex> coefficients, Integrator_1D &integrator) :
    DiscreteFunctionMesh(mesh, coefficients),
    _size{_mesh.numElements()},
    _integrator{integrator}
{
 ...
 * [0,1] parametrization.
 */
BEM::Complex RegularP1_L0Mesh_1D::P1Function::evaluate(unsigned elementIndex, [[maybe_unused]]double t) const
{
    assert(elementIndex < _mesh.numElements() and elementIndex >= 0);
    BEM::Complex a = elementIndex > 0 ? _coefficients[elementIndex - 1] : 0.0;
    BEM::Complex b = _coefficients[elementIndex];
    return a+(b-a)*t;
}
 ...
 * @brief: return a function evaluating then derivative of the corresponding function. 
 */
DiscreteFunctionMesh &RegularP1_L0Mesh_1D::P1Function::derivative(void) const
{
    std::lock_guard<std::mutex> lock(_derMutex);
    if (not _derivative) {
        auto vec1 = _coefficients;
 ...
 * @brief: Construct a basis function
 */
RegularP1_L0Mesh_1D::BasisFunction::BasisFunction(const Mesh1D &mesh, unsigned index, Integrator_1D & integrator) :
    ...
    P1Function(mesh, BEM::basisVector(index, mesh.numElements()), integrator)
{
}
 ...
 * to the point of evaluation, second one to constant (since derivative of constant is 0).
 */
const ScalarFunctionBase_1D &RegularP1_L0Mesh_1D::BasisFunction::antiDerivative(unsigned elementIndex) const
{
    assert(false and "Not Implemented");
    std::lock_guard<std::mutex> lock(_derMutex);
    if (_antiDerivatives.find(elementIndex) == _antiDerivatives.end()) {
