...
public:
    ...
    RiemannLiouvilleMesh(const DiscreteSpaceMesh &space, Side side, int order);

    template <typename T, typename S>
    RiemannLiouvilleMesh(const DiscreteSpaceMesh &space, Side side, int order, T &dFun, S &qFun):
        RiemannLiouvilleMesh(space, side, order)
    {
        _dFun.reset(new T(dFun));
        _qFun.reset(new S(qFun));
    };

    RiemannLiouvilleMesh(const DiscreteSpaceMesh &space, Side side, int order, std::shared_ptr<ScalarFunctionBase_1D> &dFun, std::shared_ptr<ScalarFunctionBase_1D> &qFun):
        RiemannLiouvilleMesh(space, side, order)
    {
        _dFun = dFun;
        _qFun = qFun;
...
private:
    ...
    std::shared_ptr<ScalarFunctionBase_1D> _qFun=nullptr;
};
